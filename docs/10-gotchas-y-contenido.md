...
- **No resetear expand/scroll en cada Realtime**: `inconsistencies` es un array nuevo en cada upsert de allocations. Un `useEffect` dependiente de ese array que haga `setExpandedProjects(...)` con > `COHERENCE_AUTO_EXPAND_MAX` **colapsa toda la lista** y, si un proyecto de arriba sale de la vista, el scroll se queda en el mismo `scrollY` y el usuario pierde el proyecto que estaba leyendo. Auto-expandir solo al cambiar mes/filtros de contexto; anclar el scroll al ítem visible (`listScrollAnchor.ts`).
