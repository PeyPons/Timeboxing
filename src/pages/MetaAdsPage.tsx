...
export default function MetaAdsPage() {
  ...
  const handleStartSync = async () => {
    ...
    try {
      ...
      const channel = supabase.channel(`meta-sync-${jobId}`).on(
          'postgres_changes',
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6 pb-20">
        <div className="flex flex-col gap-4">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-3">
                    <div className="p-3 bg-[#1877F2] rounded-xl shadow-lg shadow-blue-900/10"><Facebook className="w-6 h-6 text-white" /></div>
                    <div>
                ...
                <div className="flex gap-2 w-full md:w-auto">
                    ...
                    <Button onClick={handleStartSync} disabled={isSyncing} className="bg-[#1877F2] hover:bg-blue-700 text-white w-full md:w-auto"><RefreshCw className={`w-4 h-4 mr-2 ${isSyncing ? 'animate-spin' : ''}`} /> Sincronizar</Button>
                </div>
            </div>
        ...
        <Dialog open={isSyncing} onOpenChange={(open) => { if(syncStatus !== 'running') setIsSyncing(open); }}>
          <DialogContent className="sm:max-w-md bg-slate-950 text-slate-100 border-slate-800">
            <DialogHeader><DialogTitle className="flex items-center gap-2 text-white">{syncStatus === 'running' && <RefreshCw className="w-5 h-5 animate-spin text-blue-500" />} Sincronizando (Meta Ads)</DialogTitle><DialogDescription className="text-slate-400">{syncStatus === 'running' ? `Progreso: ${Math.round(syncProgress)}%` : 'Finalizado.'}</DialogDescription></DialogHeader>
