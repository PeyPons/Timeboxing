...
export function PlannerGrid() {
  ...
  // ✅ MODIFICADO: Al hacer click, guardamos la fecha "REAL" de la semana, pero AllocationSheet usará currentMonth para saber la llave
  ...
  const handleAnalyze = async () => {
    ...
    if (!apiKey) {
        setInsights([{ type: 'warning', text: '⚠️ Falta la API Key de Gemini en .env' }]);
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-950 rounded-lg border shadow-sm overflow-hidden">
                        ...
                        {!isAnalyzing && !insights && (
                            <div className="flex flex-col items-center gap-3 py-2 text-center">
                                <p className="text-sm text-muted-foreground">
                                    Analiza la carga de trabajo actual para detectar cuellos de botella.
                                </p>
                    ...
                    return (
                        <div key={employee.id} className="grid border-b hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors group bg-white dark:bg-slate-950" style={{ gridTemplateColumns: gridTemplate }}>
                            <EmployeeRow 
                                ...
                                currentMonth={currentMonth} // ✅ CRUCIAL
                            />
      ...
      {selectedCell && (
        <AllocationSheet 
            ...
            viewDateContext={currentMonth} // ✅ CRUCIAL
        />
      )}
    </div>
  );
